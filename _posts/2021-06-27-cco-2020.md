...
My code is available [here](https://github.com/crackersamdjam/Programming-Contest-Solutions/tree/master/CCO). Remember to **not** copy and paste code on DM::OJ.
...
N-2 + \ceil{\frac{N}{2}} &< 2N-2 - (N + \lceil \frac{N}{2} - N \rceil ) \\
...
This problem uses a technique called [fracturing search](https://algorithms-live.blogspot.com/2018/08/episode-32-fracturing-search.html). It’s very similar to using a modified Dijkstra’s algorithm to find the $K^\text{th}$ cheapest plan, but the problem with a naive implementation is the number of states and transitions we need might be huge. We want an $\mathcal{O}(1)$ transition and $\mathcal{O}(K)$ states.
